...
import { addFilm, deleteFilm, fetchFilms, updateFilm } from "@/stores/api";
...
const Admin = () => {
    ...
    const handleAddFilm = (newFilm) => {
        dispatch(addFilm(newFilm)).then(() => {
            dispatch(fetchFilms()); // Refresh data setelah tambah
        });
    };

    const handleUpdateFilm = (id, updatedFilm) => {
        dispatch(updateFilm({ id, updatedFilm })).then(() => {
            dispatch(fetchFilms()); // Refresh data setelah update
        });
    };

    const handleDeleteFilm = (id) => {
        dispatch(deleteFilm(id)).then(() => {
            dispatch(fetchFilms()); // Refresh data setelah hapus
        });
    };
    ...
    const openEditModal = (id) => {
        const rowToEdit = films.find((row) => row.id === id);
        setModalMode("edit");
        setModalData(rowToEdit); // Set data yang akan diedit
        setIsModalOpen(true);
    };
    ...
    const handleModalSubmit = () => {
        if (modalMode === "add") {
            handleAddFilm(modalData);
        } else if (modalMode === "edit") {
            handleUpdateFilm(modalData.id, modalData);
        }
        closeModal2();
    };
